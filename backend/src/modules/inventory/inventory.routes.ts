...
export async function registerInventoryRoutes(app: FastifyInstance): Promise<void> {
  app.get('/categories', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_CATEGORIES)] }, async (request) => {
    return listCategories(resolveTenantId(request));
  });

  app.post('/categories', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_CATEGORIES)] }, async (request) => {
  ...
  app.patch('/categories/:id', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_CATEGORIES)] }, async (request) => {
  ...
  app.delete('/categories/:id', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_CATEGORIES)] }, async (request) => {
    const { id } = request.params as { id: string };
  ...
  app.get('/products/barcode/:barcode', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_VIEW)] }, async (request) => {
    const { barcode } = request.params as { barcode: string };
    return getProductByBarcode(resolveTenantId(request), decodeURIComponent(barcode));
  });

  app.post('/products', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_EDIT)] }, async (request) => {
    const parsed = productSchema.safeParse(request.body);
    if (!parsed.success) throw new ValidationError('Invalid request body', parsed.error.flatten());
    return createProduct(resolveTenantId(request), parsed.data);
  });

  app.patch('/products/:id', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_EDIT)] }, async (request) => {
    const { id } = request.params as { id: string };
  ...
  app.delete('/products/:id', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_EDIT)] }, async (request) => {
    const { id } = request.params as { id: string };
    return deleteProduct(resolveTenantId(request), id);
  });

  app.post('/products/import', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_EDIT)] }, async (request) => {
    const parsed = importProductsSchema.safeParse(request.body);
    if (!parsed.success) throw new ValidationError('Invalid request body', parsed.error.flatten());
    return importProducts(resolveTenantId(request), parsed.data);
  });

  app.delete('/products', { preHandler: [authenticate, requireFeature(FEATURES.INVENTORY_EDIT)] }, async (request) => {
