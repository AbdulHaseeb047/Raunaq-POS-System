  ...
  return (
    <div className="mx-auto max-w-[300px] rounded-lg border border-border bg-white p-4 font-sans text-[11px] leading-relaxed shadow-sm">
      <div className="border-b-2 border-text pb-2 text-center">
        ...
        <p className="mt-1 text-[10px] font-bold uppercase text-text-muted">Purchase / Stock-in Slip</p>
      </div>
      ...
      {stock && (
        <div className="mt-3 border-t border-dashed border-border pt-2">
          <p className="mb-1 text-[9px] font-bold uppercase tracking-[0.18em] text-text-muted">Items received</p>
          <p className="font-semibold text-text">{stock.productName}</p>
      ...
      {(entry.notes || stock?.notes) && (
        <div className="mt-3 border-t border-dashed border-border pt-2">
          <p className="text-[9px] font-bold uppercase text-text-muted">Notes</p>
          <p className="text-[10px] text-text-muted">{entry.notes ?? stock?.notes}</p>
        </div>
      )}
    </div>
  );
